# auth — งานที่ต้องทำ (session revoke + access token generation)

> **สถานะ auth (แพ็กเกจ):** O-16 + **D1 Redis publish** implemented ตั้งแต่ v0.1.4 — gateway/staff caller ยังเป็นงานแยก (ดู §7 และ gateway doc)

เอกสารนี้สรุป **งานออกแบบ** สำหรับ:
...
คู่มือ SoT หลักของ auth ยังคือ [`architecture.md`](./architecture.md) และ [`openapi.yaml`](../openapi.yaml) — เมื่อ implement แล้วต้อง sync เอกสารเหล่านั้น + registry error ใน `_coding-standards/auth/codes.yaml`
...
| SoT เอกสาร | อัปเดต [`architecture.md`](./architecture.md) section **8.3** (ตาราง `auth_users`, diagram ถ้ามี)       |
...
2026-05-15 — D1: auth publishes `user:{sub}:token_gen` เมื่อ `REDIS_URL` ตั้ง; readyz ping Redis
