#!/usr/bin/env node
/**
 * E2E: login ที่ auth → เรียก gateway พร้อม Bearer → upstream (**reference**)
 *
 * ต้องรัน concurrently:
 *   1) auth (เช่น :3001)
 *   2) reference `npm run dev` (:3003 — ดู `access/reference/.env.example`)
 *   3) gateway `npm start` (:3002 + `ROUTES_JSON` ชี้ reference สำหรับ `/api` + `/api/v1/items`, `GATEWAY_SECRET` ตรงกัน)
 *
...
if (!proxied.ok && (proxied.status === 502 || proxied.status === 504)) {
  console.error(
    '\nHint: upstream ไม่ขึ้นหรือ ROUTES_JSON ไม่ตรง path — ตัวอย่าง reference upstream:\n' +
      '  ROUTES_JSON=[{"prefix":"/api","upstream":"http://127.0.0.1:3003","stripPrefix":false}]\n' +
      'และ GATEWAY_SECRET ต้องตรงกับ reference (`GATEWAY_SHARED_SECRET`)'
  )
}
