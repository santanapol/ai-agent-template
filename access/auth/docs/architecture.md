...
| **OpenAPI**          | [`openapi.yaml`](../openapi.yaml) — ไฟล์ที่ **`npm run spec:lint`** ใช้ใน CI · [`docs/openapi.yaml`](./openapi.yaml) — SoT เดียวกันที่ขยายคำอธิบาย + ลิงก์ข้ามเอกสาร — **ควร sync คู่กัน** เมื่อเปลี่ยนสัญญา HTTP |
...
สเปก HTTP แบบ machine-readable อยู่ที่ OpenAPI: **`openapi.yaml`** ที่ root ของแพ็กเกจคือไฟล์ที่ CI ใช้ **`spectral lint`** (`npm run spec:lint`) และ **`docs/openapi.yaml`** เป็นเลเยอร์สัญญาเดียวกันที่ขยายคำอธิบาย normative, ลิงก์ไป companion docs และหมายเหตุ tooling — เมื่อเปลี่ยน path / schema **ควร** อัปเดตทั้งคู่ให้สอดคล้องกับ section **4–5** ในเอกสารนี้
