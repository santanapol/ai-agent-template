...
- **`frontend/backoffice-shadcn/`:** README and `.env.local.example` for local dev setup (port 5175, per-service backend install).

### Changed

- **`frontend/backoffice-shadcn/`:** Production page title; stable `useAppFeedback` return value to stop Dashboard stat fetch loop and toast spam on load.
