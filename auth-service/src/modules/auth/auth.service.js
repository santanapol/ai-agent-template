...
export class AuthService {
  ...
  async login ({ username, password, client_kind, ip, request_id }) {
    ...
    const user = await this.repo.findUserByUsernameNormalized(un)
    ...
    return {
      ...
      body,
      cookie: useCookie
        ? { name: this.env.REFRESH_COOKIE_NAME, value: refreshPlain }
        : null
    }
  }
  ...
  async refresh ({ rawRefresh, refreshChannel, ip, request_id }) {
    ...
    try {
      try {
        await session.withTransaction(async () => {
          await this.rotateRefreshTokenTxnBody(
            { hash, now, newHash, expires_at, row },
            session
          )
        })
      } catch (e) {
    ...
    return {
      ...
      status: 200,
      body,
      cookie: useCookieChannel
        ? { name: this.env.REFRESH_COOKIE_NAME, value: newPlain }
        : null
    }
  }
