...
This file is the **single service-level design summary** for the `services/.demo/crud-service` sample upstream (Express + MongoDB) in the `zero-platform` monorepo. It complements the HTTP contract in the repository root [`openapi.yaml`](../openapi.yaml).
