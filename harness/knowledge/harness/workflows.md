...
2. [harness/knowledge/harness/README.md](./README.md) — วิธีทำงาน
...
รันเมื่อ: clone ใหม่, upstream agent-skills อัปเดต, หรือแก้ `harness/scripts/agent/agent-skills-standards/`
...
| ขั้น | ผลลัพธ์ |
|------|---------|
| Sync skills/agents/references | `.cursor/skills/`, `.cursor/agents/`, `harness/references/ ถูกทับ |
| แปลง commands | upstream → `.cursor/commands/` + Related Coding Standards |
| Copy local commands | `harness/scripts/agent/local-commands/*.md` → `.cursor/commands/` |
| Regenerate meta | `.cursor/rules/agent-skills.mdc`, `VENDOR.md`, `USAGE.md` |

**ต้องการแก้พฤติกรรม command?** แก้ที่ `harness/scripts/agent/agent-skills-standards/<cmd>.md` แล้ว sync ใหม่ — **อย่าแก้** `.cursor/commands/` ตรง ๆ
...
Agent อ่าน `docs/specs/` (+ `coding-standard/` เมื่อ vendor แล้ว) แล้วเขียน/อัปเดต spec ที่ **`docs/specs/<slug>.md`** (ห้าม `SPEC.md` ที่ root)
...
1. `node harness/scripts/ci/docs-lint.mjs`
...
| Agent ops | [HARNESS-RUNBOOK.md](../../HARNESS-RUNBOOK.md) |
