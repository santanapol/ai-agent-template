...
# Usage: ./scripts/ci-all.sh [--skip-install] [--skip-smoke] [--with-frontend] [--no-obs] [--only backend|frontend|docs|smoke]
set -uo pipefail
...
usage() {
  cat <<'EOF'
Usage: ./scripts/ci-all.sh [options]
...
run_docs_lint() {
  echo ""
  echo "==> Docs lint"
  if node "$ROOT/scripts/docs-lint.mjs"; then
...
run_smoke_phase() {
  ...
  if ! "$SCRIPT_DIR/dev-up.sh" "${dev_up_args[@]}"; then
    record_failure "smoke/dev-up"
    smoke_failed=true
  elif ! "$SCRIPT_DIR/smoke.sh"; then
    record_failure "smoke/smoke.sh"
    smoke_failed=true
  fi

  if ! "$SCRIPT_DIR/dev-down.sh"; then
