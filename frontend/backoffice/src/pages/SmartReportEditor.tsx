...
export interface SmartReportEditorProps {
  ...
  testRunPreviewTable: { columns: { key: string; title: string; dataIndex?: string }[]; rows: Record<string, unknown>[] };
        ...
        <Card>
          ...
          <CardContent>
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2 rounded-t-lg border border-b-0 bg-muted/30 px-3 py-2">
              <Tabs
                value={editorTab}
                onValueChange={(value) => onEditorTabChange(value as EditorTab)}
              >
                <TabsList>
