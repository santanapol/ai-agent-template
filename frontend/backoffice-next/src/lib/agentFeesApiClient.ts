...
export async function listAgentFees(agentId: string, params: ListFeesParams = {}, signal?: AbortSignal) {
  const res = await client.get<ApiEnvelope<AgentFee[]>>(`/api/v1/agent-invoice/agents/${agentId}/fees`, {
    params,
    signal,
  });
  return {
...
export async function updateAgentFee(agentId: string, feeId: string, payload: UpdateFeePayload, dateISO: string) {
  ...
  const res = await client.patch<ApiEnvelope<unknown>>(
    `/api/v1/agent-invoice/agents/${agentId}/fees/${feeId}`,
    payload,
    {
