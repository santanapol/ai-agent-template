import type { MenuNode, TokenResponse } from "../types/auth";
...
export async function listMyBranches(options?: {
  q?: string;
  limit?: number;
  offset?: number;
  signal?: AbortSignal;
...
export async function updateAdminMenu(
  ...
  const res = await authClient.patch<AdminMenuNode>(`/auth/admin/menus/${encodeURIComponent(key)}`, payload, {
...
export async function listRolePermissions(
  ...
  const res = await authClient.get<{ role_permissions: RolePermissionMapping[] }>("/auth/admin/role-permissions", {
    params: query,
  });
...
export async function deleteRolePermission(role: string, options?: { confirm?: boolean }): Promise<void> {
  await authClient.delete(`/auth/admin/role-permissions/null/${encodeURIComponent(role)}`, {
