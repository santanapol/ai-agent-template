...
export function getTestRunDateTagLabel(script: string | undefined, runParams?: TestRunRunParams): string | null {
...
export function scriptRequiresGate(
  ...
  return editingReport === null || (baselineScript !== null && (queryValue ?? "") !== baselineScript);
}
...
export function canSaveScript(
  ...
  return !requiresGate || (gateStatus === "tested" && !!testRunToken && !!compiledScript);
}

/** Hint shown when Save is disabled due to the script gate. */
export function getSaveGateHint(requiresGate: boolean, gateStatus: ScriptGateStatus): string | null {
...
export function isEditorDirty(current: EditorSnapshot, baseline: EditorSnapshot | null): boolean {
  ...
  return normalizeFormValuesForCompare(current.formValues) !== normalizeFormValuesForCompare(baseline.formValues);
}
...
export function normalizeTestRunSample(sample: Record<string, unknown>[]): {
  columns: string[];
  rows: TestRunPreviewRow[];
} {
  if (sample.length === 0) {
    return { columns: [], rows: [] };
  }
