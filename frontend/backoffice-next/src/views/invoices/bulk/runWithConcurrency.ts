...
export async function runWithConcurrency(options: RunWithConcurrencyOptions): Promise<BulkResultItem[]> {
  const { invoiceIds, concurrency = DEFAULT_BULK_CONCURRENCY, signal, onProgress, processInvoice } = options;
