    ...
    <Dialog open={open} onOpenChange={(next) => !next && !running && onClose()}>
      <DialogContent className="sm:max-w-lg" showCloseButton={!running}>
        <DialogHeader>
          ...
          <DialogDescription>
            Track bulk invoice operation progress. Results update as each item completes.
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4" role="status" aria-live="polite">
          ...
          <ul className="flex max-h-64 flex-col gap-2 overflow-y-auto">
            {progress.results.length === 0 ? (
              <li className="text-muted-foreground text-sm">Waiting to start…</li>
            ) : (
              progress.results.map((item) => (
                <li key={item.id} className="flex gap-2 text-sm">
                  <span className="font-medium">
                    {item.status === "success" ? "Success" : item.status === "failed" ? "Failed" : "Cancelled"}
                  </span>
                  {item.status === "success" ? (
