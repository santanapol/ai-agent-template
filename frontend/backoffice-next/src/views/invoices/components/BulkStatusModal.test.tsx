...
describe("BulkStatusModal", () => {
  ...
  it("shows progress and clears selection when all updates succeed", async () => {
    ...
    const onClose = vi.fn();
    render(<BulkStatusModal open invoiceIds={["inv1"]} action="PAID" onClose={onClose} />);
  ...
  it("keeps running state until update finishes after cancel is clicked", async () => {
    let resolveUpdate:
      | ((value: { successCount: number; failedCount: number; cancelledCount: number }) => void)
      | undefined;
    const updatePromise = new Promise<{ successCount: number; failedCount: number; cancelledCount: number }>(
      (resolve) => {
    ...
    render(
      <BulkStatusModal open invoiceIds={["inv1"]} action="PAID" onClose={vi.fn()} onRunningChange={onRunningChange} />,
    );
  ...
  it("retries only failed items", async () => {
    ...
    render(<BulkStatusModal open invoiceIds={["inv1", "inv2"]} action="VOID" onClose={vi.fn()} />);
  ...
  it("does not clear selection when closing after partial success", async () => {
    ...
    const onClose = vi.fn();
    render(<BulkStatusModal open invoiceIds={["inv1", "inv2"]} action="PAID" onClose={onClose} />);
