...
export function ConfirmDialogProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <ConfirmDialogContext.Provider value={{ confirm }}>
      ...
      >
        <AlertDialogContent className="sm:max-w-md" aria-describedby={options.content ? descriptionId : undefined}>
          <AlertDialogHeader>
            <AlertDialogTitle>{options.title ?? "Confirm"}</AlertDialogTitle>
            {options.content ? (
              <AlertDialogDescription id={descriptionId}>{options.content}</AlertDialogDescription>
            ) : null}
          </AlertDialogHeader>
          <AlertDialogFooter>
