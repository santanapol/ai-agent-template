...
function ThemePresetValue({
  ...
  const preset = THEME_PRESET_OPTIONS.find((option) => option.value === value);
...
function FontValue({ value }: { value: FontKey | null }) {
  if (!value) return null;
  return fontOptions.find((option) => option.key === value)?.label ?? value;
}
...
export function LayoutControls() {
  ...
  const onThemePresetChange = (preset: ThemePreset | null) => {
    if (!preset) return;
    setPreference("theme_preset", preset);
  };

  const onThemeModeChange = (next: string[]) => {
    const mode = next[0] as ThemeMode | undefined;
    if (mode) setPreference("theme_mode", mode);
  };

  const onContentLayoutChange = (next: string[]) => {
    const layout = next[0] as ContentLayout | undefined;
    if (layout) setPreference("content_layout", layout);
  };

  const onNavbarStyleChange = (next: string[]) => {
    const style = next[0] as NavbarStyle | undefined;
    if (style) setPreference("navbar_style", style);
  };

  const onSidebarStyleChange = (next: string[]) => {
    const value = next[0] as SidebarVariant | undefined;
    if (value) setPreference("sidebar_variant", value);
  };

  const onSidebarCollapseModeChange = (next: string[]) => {
    const value = next[0] as SidebarCollapsible | undefined;
    if (value) setPreference("sidebar_collapsible", value);
  };

  const onFontChange = (value: FontKey | null) => {
    if (!value) return;
  ...
  return (
    <Popover>
      <PopoverTrigger render={<Button type="button" size="icon" aria-label="Layout preferences" />}>
              ...
              <Select value={themePreset} onValueChange={onThemePresetChange}>
                <SelectTrigger size="sm" className="w-full text-xs">
                  <SelectValue placeholder="Preset">
                    {(value) => (
                      <ThemePresetValue value={value as ThemePreset | null} resolvedThemeMode={resolvedThemeMode} />
                    )}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
              ...
              <Select value={font} onValueChange={onFontChange}>
                <SelectTrigger size="sm" className="w-full text-xs">
                  <SelectValue placeholder="Select font">
                    {(value) => <FontValue value={value as FontKey | null} />}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
