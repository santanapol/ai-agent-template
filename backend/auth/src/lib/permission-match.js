/**
 * Permission Matching Contract (SPEC: Dynamic Permission in DB).
 * Entry ครอบคลุม action key เมื่อ: exact match หรือ wildcard รูปแบบเดียวคือ `domain:*`
 * Contract นี้ gateway/upstream services ต้อง implement ให้ตรงกัน — ห้ามแก้ฝั่งเดียว
 */
...
export function matchesPermission(entry, actionKey) {
  ...
  if (isWildcardEntry(entry)) {
    ...
    return (
      actionKey.startsWith(prefix) && actionKey.length > prefix.length && !actionKey.includes('*')
    )
  }
