...
- **Permission Admin API (Phase A)** — จัดการ `auth_menus`/`auth_role_permissions` runtime ผ่าน REST แทนการแก้ seed data (ดู `_mission-control/SPEC-permission-admin-api.md`):
  - **`src/lib/permission-validation.js`**: แยกกฎ validate 7 ข้อ (เดิมอยู่ใน `seed-permissions.js`) ออกเป็น shared module ใช้ร่วมกันทั้ง seed script และ Admin API
  - **`/auth/admin/menus`** (`src/modules/admin/`): `GET`/`POST`/`PATCH`/`DELETE` จัดการ menu registry — ต้องมีสิทธิ์ `permissions:manage`, optimistic locking ผ่าน `upd_date`/`If-Match`, audit log `auth.permissions_changed`, ป้องกัน self-lockout (ห้ามแก้/ลบ `permissions:manage` ผ่าน API), `409 AUTH_MENU_IN_USE` เมื่อมี children, ถูกอ้างอิงโดย role-permission mapping, หรือ wildcard จะไม่เหลือ action ใดเลยหลังลบ
  - **`/auth/admin/role-permissions`** (`src/modules/admin/`): `GET`/`PUT`/`DELETE` จัดการ mapping `(ou_id, role) -> permissions[]`; รองรับ URL `:ou_id = "null"` -> global scope; `PUT` พร้อม `revoke_sessions: true` เพื่อขยับ `token_gen` ของผู้ใช้ในสโคป + ลบ Redis session ผ่าน pipeline (batch สูงสุด 1,000 users)
  - **Error codes**: เพิ่ม `AUTH_ROLE_PERMISSION_IN_USE` และ `AUTH_MENU_IN_USE` (ทั้งคู่ 409) ใน `Problem.code` enum
  - **Tests**: ขยาย `test/admin.integration.test.js` + `test/seed-permissions.test.js` ครอบคลุม Admin API ทั้งหมด (รวม 134/134 tests)
- **Permission Admin UI (Phase F2)** — Backoffice `/permissions` สำหรับผู้มี `permissions:manage` (ดู `frontend/backoffice/_mission-control/SPEC-permission-admin-ui.md`):
