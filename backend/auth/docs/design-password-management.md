...
**Cross-package design:** [`../../services/staff/docs/design-password-management.md`](../../services/staff/docs/design-password-management.md)  
...
Gateway: client calls **`POST /auth/me/password`** via existing `/auth` → auth upstream ([`gateway/routes.json`](../../../gateway/routes.json)).
...
| Rule            | Detail                                                                      |
| :-------------- | :-------------------------------------------------------------------------- |
| Policy          | `new_password` min **16**, max **256** (align internal create)              |
...
Used by staff on create when `user_id` omitted. Body includes **`password`** (required, min 16).

Document fully in [`openapi.yaml`](../openapi.yaml) — see [`architecture.md` §4.5](./architecture.md#45-internal-api--provision-user-staff-caller).
...
4. Sync Redis `user:{sub}:token_gen` when `REDIS_URL` set (fail-closed `503` if publish fails — align O-16)

---

## 7. Error codes (registered)

Registered in `_coding-standards/auth/codes.yaml`:
...
- [`domain.md`](./domain.md) — business scope + HTTP intent index
- [`architecture.md`](./architecture.md) — normative API sections
- [`session-revoke-token-gen-changes.md`](./session-revoke-token-gen-changes.md) — `token_gen` + revoke
- [`../../services/staff/docs/domain.md`](../../services/staff/docs/domain.md) — staff orchestration
