...
| Method | Path              | Description                                                                                 |
| :----- | :---------------- | :------------------------------------------------------------------------------------------ |
| `POST` | `/internal/users` | Body: `ou_id`, `branch_id`, `username`, **`password`** (min 16), `role` → `201` + `user_id` |

**Trust boundary:** เหมือน §4.4 — Bearer `AUTH_INTERNAL_SERVICE_SECRET`, ไม่ผ่าน gateway mesh.

**OpenAPI:** ต้องมีใน [`openapi.yaml`](../openapi.yaml) (resync 2026-05-26).
...
_Document version **1.4.2** — Password management spec (`/auth/me/password`, internal set-password); O-16 session revoke implemented._
