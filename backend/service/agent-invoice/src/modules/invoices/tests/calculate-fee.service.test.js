...
test("calculateFee — logs error and returns INTERNAL_ERROR when an unexpected exception is thrown", async () => {
  ...
  assert.strictEqual(
    ...
    "log must include the invoice id",
  );
  assert.strictEqual(
    typeof errors[0].data.errName,
    "string",
    "log must include errName as a string",
  );
  assert.ok(
    !("err" in errors[0].data),
    "raw err object must not appear in the log payload",
  );
});
