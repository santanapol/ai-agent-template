...
export async function listInvoices({ query = {}, ouId }) {
  ...
  const { page: pageNum, limit: limitNum, ivNo: ivNoFilter, branchId: branchIdFilter, billingMonth: billingMonthFilter, status: statusFilter } = parsed;
  ...
  let rows = await invoiceRepo.findManyByFilter({ filter, skip, limit: fetchLimit });
