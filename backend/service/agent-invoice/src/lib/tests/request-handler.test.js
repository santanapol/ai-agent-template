...
test('handleError — maps known statusCode errors to their code without leaking message', () => {
  ...
  const error = Object.assign(new Error('DB connection string exposed'), { statusCode: 404 });
  ...
  assert.strictEqual(reply.result.body.message, error.message, 'known-status message is the thrown message');
});

test('handleError — 500 fallback must not expose error.message to client', () => {
  const reply = makeReply();
  const error = new Error('mongodb connection string: mongodb+srv://admin:secret@cluster.example.com');
  ...
  assert.ok(
    typeof reply.result.body.message === 'string' && reply.result.body.message.length > 0,
