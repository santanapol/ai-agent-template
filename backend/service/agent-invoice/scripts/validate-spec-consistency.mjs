#!/usr/bin/env node
/**
 * spec:consistency gate — agent-invoice service
 * Copied from backend-service-spec-bootstrap template; CONFIG only.
 */
...
const CONFIG = {
  specDir: resolve(PKG_ROOT, "../../../docs/specs/backend/agent-invoice"),
...
function scanProsePasswordMins(file) {
  ...
  readFileSync(file, "utf8")
    ...
    .forEach((line, i) => {
      ...
      if (m)
        hits.push({ file, line: i + 1, val: Number(m[1]), text: line.trim() });
    });
  return hits;
}
...
function checkPassword(mdFiles) {
  const hits = [
    ...
    ...(existsSync(CONFIG.openapi)
      ? scanProsePasswordMins(CONFIG.openapi)
      : []),
  ...
  if (values.length > 1) {
    ...
    for (const h of hits)
      fail(`    ${h.file}:${h.line} = ${h.val}  «${h.text}»`);
  }
}
...
async function checkRoles(mdFiles) {
  ...
  if (known.length === 0) {
    fail(
      `[roles] ${doc} §"${lines[start].trim()}" ไม่มี role identifier จาก VALID_ROLES เลย`,
    );
    fail(
      `    code VALID_ROLES = ${validRoles.join(", ")} — prose น่าจะยังใช้ชื่อเก่า`,
    );
  } else if (missing.length) {
    fail(
      `[roles] ${doc} ขาด role: ${missing.join(", ")} (มีใน code แต่ไม่มีใน prose)`,
    );
  }
}
...
async function checkCollectionValidators() {
  ...
  if (
    ...
    feesSpec &&
    !feesSpec.schema.required?.includes("agent_fee")
  ) {
    fail(
      "[validators] database-erd.md ระบุ agent_fee required แต่ module ไม่ใส่ใน required[]",
    );
  }
}
