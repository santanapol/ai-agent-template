# ADR 001: `PUT` full replace on demo-service items

## Status

Accepted — **`demo-service`** sample upstream only (under `services/.demo/`).
...
The **`demo-service`** package exists to demonstrate **full-document replace** semantics (audit fields recomputed, full body validation matching `ReplaceItem` schema) alongside **`PATCH`** for partial updates.
