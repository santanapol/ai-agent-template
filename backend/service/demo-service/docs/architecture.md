# demo-service — architecture (package SoT)
...
**TL;DR:** Client → **gateway** (Bearer JWT) → inject mesh headers → **demo-service** (`x-gateway-secret` + `x-user-*`) → MongoDB `items` / header-only `me`
