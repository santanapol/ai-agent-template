# demo-service — Database (ERD & schema)
...
**Scope:** writes **only** collection **`items`** in **`DB_NAME`** (default `demo-service`). **`GET /api/v1/me`** does not read MongoDB.
...
- **Database:** **`DB_NAME`** (default **`demo-service`**) — [`../../.env.example`](../../.env.example), [`../../src/config/env.js`](../../src/config/env.js)
...
| **`APP_NAME`** | Client metadata (default `demo-service`) |
