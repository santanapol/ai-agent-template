...
```tsx
<Card
  title={<Typography.Title level={4} style={{ margin: 0 }}>Royalty 21 Times</Typography.Title>}
  bordered={false}
>
...
const { data, isLoading, error, refetch } = useQuery({
  ...
  queryFn: async () => {
    const res = await branchReportApi.getRoyalty21Times({ ...searchParams, page, pageSize });
