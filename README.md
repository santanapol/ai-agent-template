...
> **TL;DR:** Client รับ JWT จาก **auth** แล้วเรียก **gateway** ด้วย Bearer token; **gateway** verify JWT, inject headers และ proxy ไป upstream (**[`crud-service`](./services/.demo/crud-service/README.md)**, [`smart-report`](./services/smart-report/) ตาม `ROUTES_JSON`)
...
| **5. CRUD sample (`services/.demo/crud-service`)**<br/>[`crud-service/README.md`](./services/.demo/crud-service/README.md) | ตัวอย่าง Express + Mongo หลัง gateway — **`/api/v1/me`**, **`/api/v1/items`**, และ catch-all **`/api`** → upstream `:3003` (ดู [`gateway/.env.example`](./gateway/.env.example) `ROUTES_JSON`) |
