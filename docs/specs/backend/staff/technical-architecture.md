...
| **HTTP contract**       | **`openapi.yaml`** + **`openapi-via-gateway.yaml`** — bootstrapped                                                         |
...
| sequences (create, archive, restore) | [§6](./business-domain.md#6-flow-หลัก-sequence)                                                                                                         |
...
อ้างอิง [`../../../ARCHITECTURE.md`](../../../../backend/ARCHITECTURE.md)
...
| `password` | **yes** (provision path) | min 8; ส่งต่อ `POST /internal/users` (auth policy)                                              |
...
**`token_gen` (access JWT invalidation):** SoT **auth**; gateway ตาม [`session-revoke-token-gen-changes.md`](../../../../backend/auth/docs/session-revoke-token-gen-changes.md)
...
| Set password (admin) | `POST .../profiles/{id}/password`                              | `POST /internal/users/{user_id}/password`                     |
| Set role (admin)     | `PATCH .../profiles/{id}/role`                                 | `PATCH /internal/users/{user_id}/role`                        |
| Revoke sessions      | หลัง archive หรือหลัง password reset (เมื่อ `revoke_sessions`) | `POST /internal/users/{user_id}/sessions/revoke`              |
| Deactivate user      | orphan cleanup หลัง link ล้ม (best-effort)                     | `POST /internal/users/{user_id}/deactivate` — **auth ยังไม่ implement**; client no-op |

Client: `src/lib/clients/auth-internal.client.js`

| สถานะ                 | หมายเหตุ                                                                                                                   |
| :-------------------- | :------------------------------------------------------------------------------------------------------------------------- |
| **Revoke retry**      | Mongo archive สำเร็จ + revoke ล้มหลัง retry → staff ตอบ **`200`** + archived profile + metric — ดู [`business-domain.md` §6.2](./business-domain.md#62-archive) |
| **Provision failure** | insert profile **ก่อน** auth; provision ล้ม → **delete profile** + 4xx/503 ตาม auth — ไม่ orphan auth user |
...
อ้างอิง [`../../../gateway/routes.json`](../../../../backend/gateway/routes.json) และ [`../../../README.md`](../../../../backend/README.md)
...
| Create `password` in body                                | **required** (min 8) เมื่อไม่ส่ง `user_id` — ส่งต่อ auth `POST /internal/users` (ไม่ใช้ env รหัสร่วม) |
| Gateway route                                            | ล็อกใน [`gateway/routes.json`](../../../../backend/gateway/routes.json) — prefix `/api/v1/staff`                  |
...
- [`WORKFLOW.md`](./WORKFLOW.md) — spec-driven workflow
...
- [`../../../gateway/docs/session-revoke-token-gen-changes.md`](../../../../backend/gateway/docs/session-revoke-token-gen-changes.md)
- [`../../../auth/src/config/mongo-collections.js`](../../../../backend/auth/src/config/mongo-collections.js)
- [`../../../ARCHITECTURE.md`](../../../../backend/ARCHITECTURE.md)

## Last updated

2026-07-03 — G1 drift patch: provision profile-first; archive revoke fail → 200 + metric; deactivate auth endpoint N/A
2026-07-02 — Re-audit: source layout (`plugins/` not `middlewares/`); outbound auth adds set-role + deactivate
2026-07-02 — Implemented status; central spec backlinks; `PATCH .../role` in HTTP surface
