...
## Executive summary (pre-fix snapshot)
...
**Key finding (historical):** `spec:consistency` passed on all 5 spec services but **did not detect behavioral drift** (branch scope, env var names, invoice statuses, OpenAPI vs runtime). **agent-invoice** was highest priority: central spec described pre-#46 branch behavior while code enforced `scopeBranchId` + `resolveListInvoicesRequestQuery`.

**CI gaps (repo-wide, pre-fix):**
