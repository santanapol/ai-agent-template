...
| Rate limit (HTTP) | **ต้อง** มี **middleware จำกัดความถี่แยกต่อ route** สำหรับ **`POST /auth/login`**, **`POST /auth/refresh`**, **`POST /auth/logout`** (คนละ bucket — ห้ามใช้ global เดียวกันทั้งสามเส้นถ้าทำให้ refresh กิน quota ของ login) — **implementation ปัจจุบัน (`auth.route.js`):** login **30**/นาที, refresh **120**/นาที, logout **60**/นาที ต่อ IP — SoT มาตรฐานองค์กร: [`coding-standard/auth/5-security-and-validation.md`](../../../../coding-standard/auth/5-security-and-validation.md) (§ Security) — แยกจาก **credential throttle** (คนละชั้น; **`429` `type` เดียวกัน** ใน implementation — ดู 4.3); อ้างอิงเพิ่ม: `api-rate-limit-standard.md` |
