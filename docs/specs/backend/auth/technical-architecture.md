...
| Field                | Value                                                                                                                                                                                                                                                   |
| :------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Filename**         | `technical-architecture.md` (this folder)                                                                                                                                                                                                               |
| **Entry point**      | [auth-spec.md](./auth-spec.md)                                                                                                                                                                                                                          |
| **Status**           | Active — Technical SoT (central spec folder)                                                                                                                                                                                                            |
| **Companion docs**   | [backend/ARCHITECTURE.md](../../../../backend/ARCHITECTURE.md), [gateway technical doc](../../../../backend/gateway/docs/architecture.md), [session-revoke-token-gen-changes.md](./session-revoke-token-gen-changes.md) (O-16 — implemented)         |
| **OpenAPI**          | [openapi.yaml](../../../../backend/auth/openapi.yaml) — normative HTTP (`npm run spec:lint`, `npm run spec:codes`)                                                                                                                                          |
| **Scope**            | เอกสารนี้ **ไม่** แทน `gateway` — client รับ JWT จาก service นี้ แล้วค่อยเรียก `gateway`                                                                                                                                                                    |
| **Package version**  | `0.1.8` (see `backend/auth/package.json`)                                                                                                                                                                                                               |
...
| Topic               | [business-domain.md](./business-domain.md)                       |
| :------------------ | :--------------------------------------------------------------- |
| MVP scope           | [§1](./business-domain.md#1-role-and-scope-mvp)                           |
| HTTP intent         | [§5](./business-domain.md#5-http-operations-intent--before-openapi)       |
| Sequences           | [§6](./business-domain.md#6-main-flows-sequence)                          |
| Password management | [design-password-management.md](./design-password-management.md) |
...
| **`gateway`** (SoT: [`architecture.md`](../../../../backend/gateway/docs/architecture.md)) | **ไม่** ทำ login — ทำหน้าที่ **verify** access JWT แล้ว inject headers ไปยัง upstream                                       |
...
- **ต้อง** ออก **access JWT** ที่ Gateway ตรวจได้ โดย **algorithm + JWKS ต้องสอดคล้องกับ** [`gateway` SoT ส่วน 11.3](../../../../backend/gateway/docs/architecture.md) — service นี้ล็อก **โหมด (B) asymmetric เท่านั้น (O-08)** และ **ห้าม** ใช้ shared symmetric secret แบบโหมด (A) HS256 สำหรับ access token ที่ Gateway verify
...
สเปก HTTP แบบ machine-readable มี **ไฟล์เดียว** คือ [`openapi.yaml`](../../../../backend/auth/openapi.yaml) ที่ root ของแพ็กเกจ — CI ใช้ **`spectral lint`** (`npm run spec:lint`) และ **`spec:codes`** อ่านไฟล์นี้ — เมื่อเปลี่ยน path / schema **ต้อง** อัปเดตให้สอดคล้องกับ section **4–5** ในเอกสารนี้
...
| `permissions`   | Yes         | string[] — effective permission keys ณ เวลา login/refresh (deny default `[]`)                                                    |
...
| rate limit ทั่วไป (middleware `@fastify/rate-limit` — per-route bucket)                                            | **`429 Too Many Requests`** — `problem+json` **`type`** **`{PROBLEM_TYPE_BASE}/too-many-attempts`** (`rateLimit` ใน `problem.js`)                                                                                                            |
| **เฉพาะ IP** ถูก lock จาก **credential throttle** แบบ **`ip:<…>`** แต่บัญชียังไม่เข้าเงื่อนไข `423` (**ล็อก P4**) | **`429 Too Many Requests`** — **`type`** **`{PROBLEM_TYPE_BASE}/too-many-attempts`** เช่นกัน (`ipThrottle` ใน `problem.js`) — **`code`** มักเป็น **`AUTH_TOO_MANY_ATTEMPTS`**; `detail` generic (ห้ามบอกว่า username มีในระบบหรือไม่ — สอดคล้อง 4.2) |
...
**แยก semantics `429` (OBSERVED — v0.1.8):** HTTP code เดียวกันทั้ง **per-route rate limit** และ **IP credential throttle** — implementation ใช้ **`type` URI เดียวกัน** (`/too-many-attempts`) ใน `src/lib/problem.js`; client/observability **ควร** ใช้ **`code`** (`AUTH_TOO_MANY_ATTEMPTS`) และ context (route vs login throttle) แทนการแยก `type` — *ideal* แยก `type` เป็น ADR ในอนาคต

### 4.4 Internal API — revoke sessions by user (O-16 — implemented)

> **สถานะ:** implemented ในแพ็กเกจ auth (v0.1.5+) — สัญญา HTTP อยู่ที่ [`openapi.yaml`](../../../../backend/auth/openapi.yaml); checklist รายการย่อยที่ [`session-revoke-token-gen-changes.md`](./session-revoke-token-gen-changes.md)

Trusted service (เช่น **staff** หลัง archive profile สำเร็จ) เรียกตัดทุก refresh session ของผู้ใช้และ bump **`access_token_gen`** เพื่อให้ gateway ปฏิเสธ access JWT เก่าทันที (ร่วมกับ [`gateway` session-revoke doc](../../../../backend/gateway/docs/session-revoke-token-gen-changes.md))
...
### 4.5 Internal API — provision user (staff caller — implemented)

Trusted service (**staff**) สร้าง `auth_users` เมื่อสร้าง staff profile โดยไม่ส่ง `user_id`

| Method | Path              | Description                                                                                 |
| :----- | :---------------- | :------------------------------------------------------------------------------------------ |
| `POST` | `/internal/users` | Body: `ou_id`, `branch_id`, `username`, **`password`** (min **8**, complexity ตาม validator), `role` → `201` + `user_id` |

**Trust boundary:** เหมือน §4.4 — Bearer `AUTH_INTERNAL_SERVICE_SECRET`, ไม่ผ่าน gateway mesh.

**OpenAPI:** ต้องมีใน [`openapi.yaml`](../../../../backend/auth/openapi.yaml) (resync 2026-05-26).

### 4.6 Internal API — set password (staff admin reset — implemented)

| Method | Path                                 | Description                                             |
| :----- | :----------------------------------- | :------------------------------------------------------ |
| `POST` | `/internal/users/{user_id}/password` | ตั้งรหัสใหม่; optional `revoke_sessions` (default true) |

Normative detail: [`design-password-management.md` §4](./design-password-management.md#4-post-internalusersuser_idpassword-admin-reset).

### 4.7 Edge API — change own password (implemented)

| Method | Path                | Description                                                |
| :----- | :------------------ | :--------------------------------------------------------- |
| `POST` | `/auth/me/password` | Bearer access JWT; body `current_password`, `new_password` |

Client เรียกผ่าน gateway prefix `/auth` — gateway verify JWT แล้ว forward ไป auth (ไม่ inject mesh headers แบบ staff).

Normative detail: [`design-password-management.md` §3](./design-password-management.md#3-post-authmepassword-self-service).

---

## 5. JWT contract (สอดคล้อง gateway)

รายละเอียด Algorithm (RS256) และการจัดการ JWKS อยู่ที่ ADR เพื่อประสานกับ Gateway:

👉 **[`002-jwt-jwks-contract.md`](../../../../backend/auth/docs/adrs/002-jwt-jwks-contract.md)**

### 5.1 Access token claims (OBSERVED — `src/lib/jwt-access.js` `signAccessJwt()`)

payload ที่ยิงจริงทุก access token (RS256, header `kid`):

| Claim            | Source                     | Description                                                              |
| :--------------- | :------------------------- | :----------------------------------------------------------------------- |
| `sub`            | `auth_users._id`           | user id (`setSubject`)                                                    |
| _role claim_     | `JWT_CLAIM_ROLE` (env)     | ชื่อ claim ตั้งได้ (default map ไป `x-user-role` ของ gateway); ค่า = `role` |
| `ou_id`          | `auth_users.ou_id`         | tenant OU                                                                |
| `branch_id`      | active branch              | สาขาปัจจุบัน (เปลี่ยนได้ผ่าน branch switch)                              |
| `home_branch_id` | `auth_users.branch_id`     | สาขาต้นสังกัด (คงที่)                                                    |
| `token_gen`      | `auth_users.access_token_gen` | immediate-revocation counter (O-16) — gateway เทียบกับ Redis/DB          |
| `permissions`    | effective permission keys  | string[] — exact + wildcard `domain:*` (ไม่ expand เพื่อคุมขนาด token); deny default `[]` |

> **หมายเหตุ:** ADR 002 สรุปเฉพาะ `sub`/`role`/`token_gen` (contract หลักกับ gateway) — ตารางนี้คือ payload เต็มตาม code; `ou_id`/`branch_id`/`home_branch_id`/`permissions` เป็น claim เสริมที่ downstream ใช้ได้
...
| :------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Runtime**    | Node **>=24** — สอดคล้อง [gateway SoT section 12.7](../../../../backend/gateway/docs/architecture.md)                                                                                                                                                                              |
...
| Rate limit (HTTP) | **ต้อง** มี **middleware จำกัดความถี่แยกต่อ route** สำหรับ **`POST /auth/login`**, **`POST /auth/refresh`**, **`POST /auth/logout`** (คนละ bucket — ห้ามใช้ global เดียวกันทั้งสามเส้นถ้าทำให้ refresh กิน quota ของ login) — **implementation ปัจจุบัน (`auth.route.js`):** login **30**/นาที, refresh **120**/นาที, logout **60**/นาที ต่อ IP — SoT มาตรฐานองค์กร: [`coding-standard/auth/05-security-and-validation.md`](../../../../coding-standard/auth/05-security-and-validation.md) (§ Security) — แยกจาก **credential throttle** (คนละชั้น; **`429` `type` เดียวกัน** ใน implementation — ดู 4.3); อ้างอิงเพิ่ม: `api-rate-limit-standard.md` |
...
- **ล็อก O-10:** แหล่งความจริงของ user คือ **MongoDB** — รายละเอียด **schema / index / TTL** อยู่ที่ **[`database-erd.md`](./database-erd.md)**
...
| `NODE_ENV`                                               | `development` / `test` / `production` — ควบคุม log format, Redis requirement, test fallbacks                                                                                                                                                                |
| `APP_NAME`                                               | Service label ใน logs                                                                                                                                                                                                                                       |
| `TRUST_PROXY`                                            | `true` เมื่ออยู่หลัง reverse proxy — ให้ `request.ip` ถูกต้อง                                                                                                                                                                                               |
...
| `JWKS_PUBLIC_URL`                                        | URL เต็ม (HTTPS) ของ JWKS — **ล็อก P2:** **ต้อง** ลงท้ายด้วย **`/.well-known/jwks.json`** และ **ต้อง** ตรงกับ **`JWT_JWKS_URL`** บน Gateway ([gateway SoT section 11.3](../../../../backend/gateway/docs/architecture.md)) — ถ้าใช้ path อื่น **ต้อง** ADR + sync Gateway |
...
| `REDIS_URL`                                              | **Required ใน production** (`NODE_ENV=production`); dev/CI ว่างได้ — เมื่อตั้ง: เชื่อม Redis; หลัง **login / refresh สำเร็จ** และหลัง **revoke / password / branch switch / admin bulk revoke** → **`SET`** `user:{sub}:token_gen` (**ล้มเหลว → 503** บน path ที่ fail-closed, 503/401 ตาม endpoint); **`GET /readyz`** รวม **`PING`** Redis                                |
| `MONGODB_URI_READ`                                       | Optional — branch master read (`gpp_777ww.su_branch`); เมื่อตั้ง → **`GET /readyz`** probe **`branch-read-mongodb`** (`app.js`)                                                                                                                          |

---

## 10. Observability & operations

- **ต้อง** มี `GET /healthz` (liveness) และ `GET /readyz` (readiness; ไม่ต้อง auth) — **`readyz`** **ต้อง** ping MongoDB; **ถ้า** ตั้ง **`REDIS_URL`** แล้ว **ต้อง** `PING` Redis ด้วย; **ถ้า** ตั้ง **`MONGODB_URI_READ`** แล้ว **ต้อง** ping branch-read Mongo เป็น dependency **`branch-read-mongodb`** (ล้มเหลว → **503** เช่นกัน)
...
- **ล็อก O-15:** path ต่อไปนี้ **ห้าม** expose สู่ public internet หากไม่มีชั้น auth/network แยก (เช่น private network, mTLS, VPN หรือ reverse proxy ภายในเท่านั้น): **`/internal/*`**, **`/auth/admin/*`**, **`/metrics`** (ถ้าเปิด metrics endpoint แยกจาก scrape ภายใน)
...
| O-09 | Stack: **Express + CommonJS** vs **Fastify + ESM** (+ ADR ถ้าผิดมาตรฐานทีม)   | 6                                                  | **ล็อกแล้ว:** **Fastify + ESM** + **ADR** ([`001-fastify-esm.md`](../../../../backend/auth/docs/adrs/001-fastify-esm.md))                                                                                                                                                                                                                                                                                           |
| O-10 | User store (เช่น MongoDB / PostgreSQL) + migration / index หลัก               | 8.1, **8.3**, **[`database-erd.md`](./database-erd.md)**  | **ล็อกแล้ว:** **MongoDB** — collections / indexes ตาม **[`database-erd.md`](./database-erd.md)**                                                                                                                                                                                                                                                                                                         |
...
| O-12 | Account lockout: ต่อ **IP** / ต่อ **user** / ทั้งคู่ + threshold + duration   | 7, 4.3, **[`database-erd.md`](./database-erd.md)**        | **ล็อกแล้ว:** **ทั้ง IP และ user** — **10 ครั้ง** / **15 นาที** (rolling) → lock **30 นาที** — **P1:** username ไม่พบ → นับเฉพาะ **`ip:`**; **P4:** user lock → **`423`**, IP-only lock → **`429`**                                                                                                                                                                                               |
...
| O-15 | path ที่ถือเป็น admin / internal — ห้าม public โดยไม่มี auth แยก              | 11, **4.4**                                        | **ล็อกแล้ว:** **`/internal/*`**, **`/auth/admin/*`**, **`/metrics`** (ถ้ามี)                                                                                                                                                                                                                                                                                                                           |
| O-16 | Access credential generation + internal revoke-by-user                        | **4.4**, 5, **[`database-erd.md`](./database-erd.md)**, 9 | **ล็อกแล้ว (implemented):** ฟิลด์ **`access_token_gen`**; claim **`token_gen`**; internal revoke + **`AUTH_INTERNAL_SERVICE_SECRET`**; **404** เมื่อ user ไม่พบ; Redis **`SET user:{sub}:token_gen`** (fail-closed เมื่อ `REDIS_URL` ตั้ง); sync หลัง login/refresh ด้วย; **`REDIS_URL` required ใน production**; gateway ตรวจ `token_gen` — [`session-revoke-token-gen-changes.md`](./session-revoke-token-gen-changes.md) |
...
| Path                                                                               | Notes                                                                             |
| :--------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------- |
| [`gateway` SoT](../../../../backend/gateway/docs/architecture.md)                                | Gateway verify, headers, JWT modes                                                |
| [`gateway` session-revoke](../../../../backend/gateway/docs/session-revoke-token-gen-changes.md) | `token_gen` verification ที่ edge (implemented)                                   |
...
| [`ARCHITECTURE.md`](../../../../backend/ARCHITECTURE.md)                                      | Trust boundary, security strategy                                                 |
...
_Document version **1.4.4** — Re-audit round 3: ADR + ARCHITECTURE link fixes._
