...
| TD-018 | Data | P3 | Orphan `agent_fees` without `agents` parent — audit script + optional cleanup | closed (2026-07-15; **accepted legacy** — prod ~1,887 rows / 30 branches retained; no delete; audit script remains for monitoring — [`agent-fees-orphan-cleanup-2026-07-13`](../ops/agent-fees-orphan-cleanup-2026-07-13.md)) |
...
| TD-039 | CI | P3 | GHA auto-deploy removed (2026-07-15) — deploy staging/prod via SSH only; see `dev-ops/*/RUNBOOK.md` | closed — superseded by SSH deploy |
| TD-040 | Frontend | P3 | `BranchSwitcher.test.tsx` flaky in CI — failed twice on PRs #74/#75 with two different assertions in the same suite, passes reliably locally (8/8) both times | closed (2026-07-15; two tests asserted on `@base-ui/react/menu` popup content synchronously after click instead of `waitFor` — wrapped in `waitFor`, matching the rest of the file; also reordered negative assertions to run after mount confirmed) |
| TD-041 | Auth/Ops | P3 | Prod `auth_menus` drift — stale `branch-report:marketing` submenu; Channel Performance nested under Marketing instead of flat under Branch Report | closed (2026-07-15; `seed-permissions.js --prune` on prod — pruned `[branch-report:marketing]`; see [`dev-ops/prod/RUNBOOK.md`](../../dev-ops/prod/RUNBOOK.md)) |
