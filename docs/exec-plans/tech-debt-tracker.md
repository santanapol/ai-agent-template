...
| TD-020 | Frontend | P3 | NET-006 Option A — remove `AdminLayout` `key={branch_id}` remount; use `branchId` context instead | open — defer [`api-network-audit-frontend`](../exec-plans/active/api-network-audit-frontend-2026-07-09.md) Phase 2 |
| TD-021 | API | P3 | Batch `GET /invoices?ids=` for bulk export (ลด N× detail fetch) | open — [`api-network-audit-backend`](../exec-plans/active/api-network-audit-backend-2026-07-09.md) future |
| TD-022 | Frontend | P3 | Smart Report drawer `listHistory(limit=100)` → paginated `limit=20` | open — [`api-network-audit-frontend`](../exec-plans/active/api-network-audit-frontend-2026-07-09.md) FE-4.1 |
