...
| TD-002 | CI | P3 | Frontend GHA only targets `frontend/backoffice` — acceptable while single frontend app exists | wontfix |
| TD-003 | Docs | P3 | Vendored `coding-standard/` drift — `scripts/check-coding-standard-sync.sh` + `/gc` checklist | closed |
